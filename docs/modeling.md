...
- The frozen-threshold column is the interesting failure. Recall collapses
  ...
  steps (Elliptic's "dark market shutdown" around step 43). **A deployed
  scorer must recalibrate its threshold per batch**, which is what the Phase 3
  replay job will do.
...
> **Known pitfall for Phase 4.** pgvector's HNSW index post-filters: it finds
> approximate neighbours first, then applies the `WHERE` clause. The reference
> pool is ~13% of rows, so a naive filtered query returns **zero rows** — this
> was reproduced during Phase 2. The fix is
> `SET hnsw.iterative_scan = relaxed_order` (pgvector 0.8+), which returns
> results identical to an exact scan. A partial index over the reference pool
> is the alternative if that proves slow.
